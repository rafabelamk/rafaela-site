...
const LOGOS = [
  ...
  { name: 'Fiori Empreendimentos', file: '/logos/fiori.png', squared: true },
  { name: 'Orsegups', file: '/logos/orsegups.png', squared: true },
  { name: 'Olhar Negócios Imobiliários', file: '/logos/olhar.png', squared: true },
]
            ...
            {LOGOS.map(logo => (
              <div key={logo.name} className="flex flex-col items-center gap-2">
                <div className="w-20 h-20 bg-white rounded-[22px] flex items-center justify-center shadow-sm hover:shadow-md transition-all hover:-translate-y-1 border border-gray-100 overflow-hidden">
                  <div className={`relative w-16 h-16 ${logo.squared ? 'rounded-xl overflow-hidden' : ''}`}>
                    <Image src={logo.file} alt={logo.name} fill className={logo.squared ? 'object-cover' : 'object-contain'} sizes="64px"/>
                  </div>
                </div>
